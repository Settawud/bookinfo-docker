...
if (process.env.SERVICE_VERSION === 'v2') {
  if (process.env.DB_TYPE === 'mysql') {
    ...
    if (process.env.MONGO_DB_USERNAME && process.env.MONGO_DB_PASSWORD) {
      var urlParts = process.env.MONGO_DB_URL.split('://')
      url = urlParts[0] + '://' + process.env.MONGO_DB_USERNAME + ':' + process.env.MONGO_DB_PASSWORD + '@' + urlParts[1] + "/" + process.env.MONGO_DB_NAME

      if (process.env.MONGO_DB_AUTH_SOURCE) {
        url += "?authSource=" + process.env.MONGO_DB_AUTH_SOURCE
      }
    }

    console.log("DEBUG: Constructed MongoDB URL: " + url.replace(process.env.MONGO_DB_PASSWORD, '***'))

    // Remove auth from options if we are putting it in URL
    if (options.auth) {
      delete options.auth;
    }

    console.log("DEBUG: Constructed MongoDB URL: " + url)
    console.log("DEBUG: MONGO_DB_NAME env var: " + process.env.MONGO_DB_NAME)
    console.log("DEBUG: MONGO_DB_URL env var: " + process.env.MONGO_DB_URL)

    console.log("MongoDB URL: " + url)
    var sanitizedOptions = Object.assign({}, options)
    if (sanitizedOptions.auth) sanitizedOptions.auth.password = '***'
    console.log("MongoDB Options: " + JSON.stringify(sanitizedOptions))
  }
}
